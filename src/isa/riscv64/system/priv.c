...
static inline void csr_write(word_t *dest, word_t src) {
  ...
  longjmp_exception(EX_II);
    ...
    *dest = src & FCSR_MASK;
    ...
    // Check whether the PMP register is out of bound.
    if (idx >= CONFIG_RV_PMP_NUM) {
      Loge("pmp number is smaller than the index, raise illegal instr exception when write pmpaddr");
      longjmp_exception(EX_II);
      return;
    }
    ...
    *dest = cfg_data;
    ...
    bool locked = cfg & SPMP_L;
    // Note that the last spmp cfg do not have next_locked or next_tor
    bool next_locked = idx == (CONFIG_RV_SPMP_NUM-1) ? false : idx < CONFIG_RV_SPMP_NUM && (spmpcfg_from_index(idx+1) & SPMP_L);
    ...
    word_t cfg_data = 0;
    for (int i = 0; i < xlen / 8; i ++ ) {
      word_t cfg = ((src >> (i*8)) & 0xff) & (SPMP_R | SPMP_W | SPMP_X | SPMP_A | SPMP_L);
